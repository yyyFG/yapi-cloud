...
`yapi-interface` 是 yApiRelay — 接口中转站中的示例接口提供方，对外提供可被调用、可测试的模拟接口，用于演示"用户在线调用接口"的完整流程：用户通过平台申请 `accessKey/secretKey` 后，在平台在线调用页或直接用 curl 请求本模块接口，体验"构造鉴权请求头 -> 服务端五步校验 -> 执行业务逻辑 -> 返回结果"的闭环。
